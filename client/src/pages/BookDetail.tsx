...
export default function BookDetail({ id }: { id: string }) {
  ...
  const { mutate: createLoanRequest, isPending: isRequesting } = useCreateLoanRequest();
  ...
  const handleAction = () => {
    if (!isAuthenticated) {
      window.location.href = `/auth/signin?redirect=/book/${id}`;
      return;
    }

    createLoanRequest(book.id, {
      onSuccess: () => {
        toast({
          title: "Request Submitted",
          description: "Your borrow request has been sent to the librarian for approval.",
        });
      },
      onError: (err) => {
        toast({
        ...
        <div className="grid md:grid-cols-[1fr_2fr] gap-12 lg:gap-20">
          ...
          >
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm">
              ...
              >
                {isRequesting ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : null}
                {isAuthenticated
