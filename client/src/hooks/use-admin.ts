...
export function useAdminLoanRequests() {
  return useQuery<AdminLoanRequest[]>({
    queryKey: ["/api/admin/loan-requests"],
    queryFn: async () => {
      const res = await fetch("/api/admin/loan-requests", { credentials: "include" });
...
export function useAdminReviewLoanRequest() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, action }: { id: string; action: "approved" | "rejected" }) => {
      const res = await fetch(`/api/admin/loan-requests/${id}/review`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        throw new Error(payload?.message || "Failed to review loan request");
      }
