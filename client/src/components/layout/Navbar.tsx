      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="hidden md:flex md:items-center md:gap-8">
            ...
            {isAuthenticated && (
              <Link href="/requests" className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors">
                My Requests
              </Link>
            )}
            {isStaff && (
            ...
            {!isLoading && (
              isAuthenticated ? (
                <div className="flex items-center gap-4">
                  <Link
                    href="/profile"
                    className="flex items-center gap-2 text-sm font-medium rounded-lg px-1.5 py-1 hover:bg-muted transition-colors"
                ...
                <div className="flex items-center gap-4">
                  <Button variant="ghost" className="hover:bg-transparent hover:text-primary" onClick={() => window.location.href = '/auth/signin'}>
                    Sign In
                  </Button>
                  <Button className="rounded-full bg-primary text-primary-foreground shadow-md hover:bg-primary/90 shadow-primary/20" onClick={() => window.location.href = '/auth/signup'}>
                    Join Library
                  </Button>
                </div>
              )
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="space-y-1 px-4 pb-6 pt-4">
              ...
              {isAuthenticated && (
                <Link href="/requests" className="block rounded-lg px-3 py-3 text-base font-medium text-foreground hover:bg-muted">
                  My Requests
                </Link>
              )}
              {isStaff && (
              ...
              <div className="mt-4 pt-4 border-t border-border">
                {isAuthenticated ? (
                  ...
                  <Button className="w-full justify-center bg-primary text-primary-foreground" onClick={() => window.location.href = '/auth/signin'}>
                    Sign In / Join
                  </Button>
                )}
              </div>
