...
import {
  ...
  useAdminUsers, useAdminUpdateRole, useAdminLoanRequests, useAdminReviewLoanRequest,
...
import {
  ...
  RotateCcw, Shield, Clock, AlertCircle, Search, ShieldCheck, Check, X
} from "lucide-react";
...
export default function Admin() {
  ...
  const { data: loanRequests, isLoading: requestsLoading } = useAdminLoanRequests();
  ...
  const reviewLoanRequest = useAdminReviewLoanRequest();
  ...
  const handleLoanRequestReview = (id: string, action: "approved" | "rejected") => {
    reviewLoanRequest.mutate(
      { id, action },
      {
        onSuccess: () => {
          toast({
        ...
        <Tabs defaultValue="books" className="space-y-6">
          <TabsList className="bg-muted/50 border border-border/50 p-1 rounded-xl">
            ...
            <TabsTrigger value="requests" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
              <Clock className="h-4 w-4 mr-2" /> Requests ({pendingRequests.length})
            </TabsTrigger>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {requestsLoading ? (
                      ...
                      <tr><td colSpan={5} className="py-8 text-center text-muted-foreground">No loan requests found</td></tr>
                    ) : (
                      loanRequests?.map((request) => (
                        <tr key={request.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                          <td className="py-3 px-4 font-medium">{request.book?.title || "Unknown"}</td>
                          <td className="py-3 px-4 text-muted-foreground">{request.userEmail || request.userId}</td>
                          <td className="py-3 px-4 text-muted-foreground hidden md:table-cell">
                            {request.requestedAt ? format(new Date(request.requestedAt), "MMM d, yyyy") : "N/A"}
                          </td>
                          <td className="py-3 px-4">
                            ...
                            {request.status === "pending" ? (
                              <div className="flex justify-end gap-2">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => handleLoanRequestReview(request.id, "approved")}
                                  disabled={reviewLoanRequest.isPending}
                                >
                                ...
                                <Button
                                  ...
                                  onClick={() => handleLoanRequestReview(request.id, "rejected")}
                                  disabled={reviewLoanRequest.isPending}
                                >
